...
const StateProvider = (props) => {
    ...
    const activeEditModal = payload => dispatch({ type: "Active_EDIT_MODAL", payload })
    const closeEditModal = () => dispatch({ type: "CLOSE_EDIT_MODAL" })
    const activeSortModal = (payload) => dispatch({ type: "ACTIVE_SORT_MODAL", payload })
    const closeSortModal = (payload) => dispatch({ type: "CLOSE_SORT_MODAL", payload })

    // FIELD HANDELAR
    const setGroupFeild = groupField => dispatch({ type: "SET_GROUP_FIELD", payload: groupField })
    const selectField = field => dispatch({ type: "SELECT_FIELD", payload: field })
    ...
    const reInitSectionValue = (value) => dispatch({ type: "RE_INIT_SECTION_VALUE", payload: value })

    // CONTENT TYPE HANDELAR
    const createContentType = ContentType => dispatch({ type: "CREATE_CONTENT_TYPE", payload: ContentType })
