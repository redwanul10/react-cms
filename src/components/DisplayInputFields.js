...
const DisplayInputFields = (props) => {
  ...
  const customHooks = useFields(
    state.renderTextEditor,
    state.isActiveTextEditor,
    toggleInput
  );
  const { extractValidation, validateValues, validations } =
    useFieldValidation(saveErrors);
      ...
      <>
        {customFields && (
          <div className="col-md-4 offset-md-5">
            <div className="inputFields flexColumn">
              {customFields.map((field, index) => (
                <div
                  ...
                  style={{ order: sortIndex[field.key] }}
                >
                  {/* <h4 >{field.title}</h4> */}
            ...
            <h2 className="btn-large" onClick={handleSubmit}>
              submit
            </h2>
          </div>
        )}
